...
t = meas(:,1);
y_meas = meas(:,2:3);      % displacement measurements

dt = t(2) - t(1);
N = length(t);

% Interpolate ground motion to measurement time (200 Hz)
...
%% MEASUREMENT MODEL (DISPLACEMENT ONLY)
% z = [y1 y2]'

H = [eye(2) zeros(2)];

% Noise variances (Table 2)
var_y = [2.642e-4  4.251e-4];
R = diag(var_y);
...
title('Story 1 Displacement (Displacement Only)');
ylabel('y_1 (m)');
legend('Q = 0','Q ≠ 0');
grid on
